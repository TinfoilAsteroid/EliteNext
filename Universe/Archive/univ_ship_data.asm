...
ProjectNodeToEye:       ld			bc,(UBnkZScaled)					; BC = Z Cordinate. By here it MUST be positive as its clamped to 4 min
...
EyePositiveXPoint:	    ld          l,ScreenCenterX						; x was positive result; 
                        ld          h,0
                        add         hl,bc								; hl = Screen Centre + X
                        jp          EyeStoreXPoint
EyeNegativeXPoint:      ld          l,ScreenCenterX                    ; x < 0 so need to subtract from the screen centre position                     
                        ld          h,0
                        ClearCarryFlag
                        sbc         hl,bc                               ; hl = Screen Centre - X
EyeStoreXPoint:         ex          de,hl                              ; also from LL62, XX3 node heap has xscreen node so far.
                        ld          (iy+0),e                            ; Update X Point TODO this bit is 16 bit aware just need to fix above bit
                        ld          (iy+1),d                            ; Update X Point
EyeProcessYPoint:       ld			bc,(UBnkZScaled)					; Now process Y co-ordinate
                        ld			a,c
                        ld			(varQ),a
...
EyePositiveYPoint:      ld          l,ScreenCenterY 					; Y is positive so above the centre line    
                        ClearCarryFlag
                        sbc         hl,bc  							 	; hl = ScreenCentreY - Y coord (as screen is 0 at top)
                        jp          EyeStoreYPoint
EyeNegativeYPoint:		ld          l,ScreenCenterY						; this bit is only 8 bit aware TODO FIX					
                        ld          h,0
                        add         hl,bc								; hl = ScreenCenterY + Y as negative is below the center of screen
EyeStoreYPoint:         ex          de,hl          ; also from LL62, XX3 node heap has xscreen node so far.
