ClippingVarsMacro:   MACRO   p?
;-- Clipping code for universe objects -------------------------------------------
p?_XX13        DB 0


p?_varK3		 DS	4				; D2
p?_centreX     equ p?_varK3
p?_varK3p2	 DB	0				; 42
p?_varK3p3	 DB	0				; 43
p?_varK3p1	 equ p?_varK3+1			; D3
p?_varK4		 DS	4				; E0
p?_centreY     equ p?_varK4
p?_varK4p1	 equ p?_varK4+1			; D3
p?_varK5       DS  6 
p?_varK5p2     equ p?_varK5+2
p?_varK6       DS  6
p?_varK6p2     equ p?_varK6+2

p?_XX12p2      DB 0    ; The line's gradient * 256 (so 1.0 = 256)
p?_XX12p3      DB 0    ; The direction of slope ; + LT to BR; - TR to BL
p?_XX12p4      DB 0
p?_XX12p5      DB 0
p?_Delta_x     EQU p?_XX12p2
p?_Delta_y     EQU p?_XX12p4
p?_Tvar        DB 0    ; The gradient of slope ; 0 if it's a shallow slope (DX > DY) ; &FF if it's a steep slope (DY > DX) Returns:  XX15        m         x1 as an 8-bit coordinate XX15+2               y1 as an 8-bit coordinate
p?_Qvar        DB 0
p?_Rvar        DB 0    ; general purpose for calcs  Paired with S must be done this way round for SUBHeightFromY1 etc to work
p?_Svar        DB 0    ; sign variable    
p?_SRvarPair   EQU p?_Rvar                
p?_Xreg        DB 0 
p?_Yreg        DB 0
p?_YXregPair   EQU p?_Xreg
                     ENDM
...
;               d     r
;               
ClippingCodeLL28Macro:      MACRO   p?
p?_LL28_6502:         ld      hl,Qvar                 ; CMP Q                  \ If A >= Q, then the answer will not fit in one byte,
                            ld      c,(hl)                  ; using c as Q var
                            cp      c
                            FlipCarryFlag
                            jp      c, p?_LL2_6502    ; BCS LL2                \ so jump to LL2 to return 255
...
p?_LL31_6502:          sla     a                       ; ASL A                  \ Shift A to the left
                            jp      c, p?_LL29_6502             ; BCS LL29               \ If bit 7 of A was set, then jump straight to the subtraction
                            FlipCarryFlag                   ;                          If A < N, then C flag is set.
                            JumpIfALTNusng c, p?_LL31_SKIPSUB_6502 ; CMP Q              \ If A < Q, skip the following subtraction
                                                                ; BCC P%+4
                            sub     c                       ; SBC Q                  \ A >= Q, so set A = A - Q
                            ClearCarryFlag
p?_LL31_SKIPSUB_6502:  FlipCarryFlag
                            ...
                            jp      c, p?_LL31_6502            ; BCS LL31               \ If we still have set bits in R, loop back to LL31 to do the next iteration of 7
                            ld      a,b
                            ld      (Rvar),a
                            ret                             ; RTS                    \ R left with remainder of division
p?_LL29_6502:          sub     c                       ; SBC Q                  \ A >= Q, so set A = A - Q
...
p?_LL2_6502:          ld      a,$FF                   ; LDA #255               \ The division is very close to 1, so return the closest
...
p?_ADDXRegtoY1:       ld      a,(Xreg)                ; Set y1 = y1 + (Y X)
                            ld      c,a
                            ld      b,0
                            ld      hl,(p?_XX1532)
                            ClearCarryFlag                
...
p?_ADDYXRegtoY1:      ld      bc,(YXregPair)          ; Set y1 = y1 + (Y X)
                            ld      hl,(p?_XX1532)
                            ClearCarryFlag                
                            adc     hl,bc
                            ld      (p?_XX1532),hl
...
p?_ADDYXRegtoX1:      ld      bc,(YXregPair)          ; Set x1 = x1 + (Y X)
                            ld      hl,(p?_XX1510)
                            ClearCarryFlag                
                            adc     hl,bc
                            ld      (p?_XX1510),hl
...
p?_SUBBCFromY1:       ld      hl,(p?_XX1532)             ; Set (S R) = (y1_hi y1_lo) - BC where BC can be say screen height 
                            ClearCarryFlag                 
                            sbc     hl,bc
                            ld      (SRvarPair),hl
                            ret

p?_AddSRToYX:         ld      hl,(YXregPair)
                            ld      de,(SRvarPair)
                            ClearCarryFlag
                            adc     hl,de
                            ld      (YXregPair),hl
...
;---------------------------------------------------------------------------------------------------------------------
ClippingCodeLL120Macro:     MACRO   p?
p?_LL120_6502:        ld      a,(p?_XX1510)              ;LDA XX15               \ Set R = x1_lo
                            ld      (Rvar),a                ;STA R
                            call    p?_LL129_6502              ;JSR LL129              \ Call LL129 to do the following:  Q = XX12+2 = line gradient, A = S EOR XX12+3 = S EOR slope direction  (S R) = |S R|
...
p?_LL122_6502:         ZeroA                           ;LDA #0                 \ Set A = 0 
                            ...
                            sla     (hl)                    ;.
                            jr      nc, p?_LL126_6502          ;BCC LL126              \ If C (i.e. the next bit from Q) is clear, do not do
; the addition for this bit of Q, and instead skip to LL126 to just do the shifts
p?_LL125_6502:        call    AddSRToYX               ;TXA                    \ Set (Y X) = (Y X) + (S R)  starting with the low bytes And then doing the high bytes
p?_LL126_6502:        ld      bc,(SRvarPair)          ;LSR S                  \ Shift (S R) to the right
                            ShiftBCRight1                   ;ROR R
                            ld      (SRvarPair),bc          ;.
                            ld      hl, Qvar                ;ASL Q                  \ Shift Q to the left, catching bit 7 in the C flag
                            sla     (hl)                    ;.
                            jr      c,p?_LL125_6502            ;BCS LL125              \ If C (i.e. the next bit from Q) is set, loop back to LL125 to do the addition for this bit of Q
                            jr      nz,p?_LL126_6502           ;BNE LL126              \ If Q has not yet run out of set bits, loop back to LL126 to do the \"shift\" part of shift-and-add until we have done additions for all the set bits in Q, to give us our multiplication result
...
;--             giving (Y X) the opposite sign to the slope direction in XX12+3.
ClippingCodeLL122Macro:      MACRO   p?
p?_LL123_6502:         call    p?_LL129_6502              ;JSR LL129              \ Call LL129 to do the following   Q = XX12+2   = line gradient
                             push    af                      ;PHA                    \ Store A on the stack so we can use it later
...
p?_LL121_6502:         ld      a,$FF                   ;LDA #%11111111         \ Set Y = %11111111
                             ld      (Yreg),a                ;TAY
...
p?_LL130_6502:         ld      bc,(SRvarPair)          ;ASL R                  \ Shift (S R) to the left
                            ShiftBCLeft1                    ;.
                            ld      (SRvarPair),bc          ;ROL S
                            ld      a,(Svar)                ;LDA S                  \ Set A = S
                            jr      c, p?_LL131_6502           ;BCS LL131              \ If bit 7 of S was set, then jump straight to the subtraction
                            ...
                            jr      nc,p?_LL132A_6502          ;BCC LL132  (NOTE Carry flag reversed in Z80 for CP)
p?_LL131_6502:         FlipCarryFlag                   ;flip carry to make it act like a 6502 borrow
                            ...
                            jp      p?_LL132_6502              ;added so that we can do a 6502 style carry above
p?_LL132A_6502:        nop; FlipCarryFlag
p?_LL132_6502:         ld      bc,(YXregPair)          ; Rotate the counter in (Y X) to the left, and catch the 
                            ...
                            jr      c, p?_LL130_6502           ; BCS LL130              \ If we still have set bits in (Y X), loop back to LL130 to do the next iteration of 15, until we have done the whole division
        ...
        ENDIF      
                            ...
                            jp      m, p?_LL128_6502           ; .                      \ subroutine with (Y X) as is
p?_LL133_6502:          ld      bc,(YXregPair)          ; TXA                    \ Otherwise negate (Y X) using two's complement by first
...
p?_LL128_6502:          ret                             ; RTS                    \ Return from the subroutine
;------------------------------------------------------------------------------------------------------- 
...
p?_LL129_6502:        push    af                      ;LDX XX12+2             \ Set Q = XX12+2
                            ld      a,(p?_XX12p2)              ;.
                            ld      (Xreg),a                ;.
                            ld      (Qvar),a                ;STX Q
                            pop     af                      ;.
...
p?_LL127_6502:        ld      hl,p?_XX12p3               ;EOR XX12+3             \ Set A = original argument S EOR'd with XX12+3
                            xor     (hl)                    ;.
                            ret                             ;RTS                    \ Return from the subroutine                    
;----------------------------------------------------------------------------------------------------------------
;--- LL118 Move along a point until on screen
...
p?_LL118_6502:         ld      a,(p?_XX1510+1)            ; LDA XX15+1             \ If x1_hi is positive, jump down to LL119 to skip the
                            and     a                       ; BPL LL119              \ .
                            jp      p, p?_LL119_6502           ;                        \ following
.X1Negative:                ld      (Svar),a                ; STA S                  \ Otherwise x1_hi is negative, i.e. off the left of the screen, so set S = x1_hi
                            call    p?_LL120_6502              ; Call LL120 to calculate:  (Y X) = (S x1_lo) * XX12+2      if T = 0   = x1 * gradient
                            ...
                            jp      p?_LL134_6502              ; just do the jump to LL134 rather than setting to equal flag and then jumping
;-- Entering LL119 a will always be the value of X1 Hi byte
p?_LL119_6502:         jp      z,p?_LL134_6502            ; BEQ LL134              \ x1_hi = 0 then jump down to LL134 to skip the following, as the x-coordinate is already on-screen
                                                            ;                        \ (as 0 <= (x_hi x_lo) <= 255)
                            ld      (Svar),a                ; STA S                  \ Otherwise x1_hi is positive, i.e. x1 >= 256 and off
                            dec     a                       ; DEC S                  \ the right side of the screen, so set S = x1_hi - 1
                            ld      (Svar),a                ;
                            call    p?_LL120_6502              ; JSR LL120              \ Call LL120 to calculate:  (Y X) = (S x1_lo) * XX12+2      if T = 0   = (x1 - 256) * gradient
                                                            ;                        \                           (Y X) = (S x1_lo) / XX12+2      if T <> 0 = (x1 - 256) / gradient
                                                            ;                        \ with the sign of (Y X) set to the opposite of the line's direction of slope
                            call    ADDYXRegtoY1             ; TXA                    \ Set y1 = y1 + (Y X)
                            ld      hl,255                  ; LDX #255               \ Set x1 = 255
                            ld      ( p?_XX1510 ),hl             ; STX XX15 ;INX; STX XX15+1
;--  We have moved the point so the x-coordinate is on  screen (i.e. in the range 0-255), so now for the  y-coordinate
p?_LL134_6502:        ld      a,(p?_XX1532+1)            ; LDA XX15+3             \ If y1_hi is positive, jump down to LL119 to skip
                            and     a                       ; BPL LL135              \ the following
                            jp      p, p?_LL135_6502           ; .
                            ld      (Svar),a                ; STA S                  \ Otherwise y1_hi is negative, i.e. off the top of the screen, so set S = y1_hi
                            ld      a, ( p?_XX1532)             ; LDA XX15+2             \ Set R = y1_lo
                            ld      (Rvar),a                ; STA R
                            call    p?_LL123_6502              ; JSR LL123              \ Call LL123 to calculate:  (Y X) = (S R) / XX12+2      if T = 0 = y1 / gradient
                                                            ;                        \                           (Y X) = (S R) * XX12+2      if T <> 0 = y1 * gradient
                                                            ;                         with the sign of (Y X) set to the opposite of the line's direction of slope
                            call    ADDYXRegtoX1            ; TXA                    \ Set x1 = x1 + (Y X)
                            ld      hl,0                    ; LDA #0                 \ Set y1 = 0
                            ld      ( p?_XX1532),hl             ; STA XX15+2, XX15+3
p?_LL135_6502:         ld      bc,128                  ; LDA XX15+2             \ Set (S R) = (y1_hi y1_lo) - screen height
                            call    SUBBCFromY1             ; .                      \ .
                            jr      c, p?_LL136_6502           ; BCC LL136              \ If the subtraction underflowed, i.e. if y1 < screen height, then y1 is already on-screen, so jump to LL136
                                                    ;                        \ to return from the subroutine, as we are done
;;-   If we get here then y1 >= screen height, i.e. off the bottom of the screen
p?_LL139_6502:         call    p?_LL123_6502              ; JSR LL123              \ Call LL123 to calculate:   (Y X) = (S R) / XX12+2      if T = 0  = (y1 - screen height) / gradient
...
;                          with the sign of (Y X) set to the opposite of the line's direction of slope
                            ...
                            ld      (p?_XX1532),hl             ; STA XX15+3             \ pixel row of the space view
p?_LL136_6502:         ret                             ; RTS                    \ Return from the subroutine
;-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;-- LL145 LINE CLIP
...
ClippingCodeLL145Macro:     MACRO   p?
p?_LL145_6502:        ZeroA                           ; LDA #0                 \ Set SWAP = 0
                            ld      (SWAP),a                ; STA SWAP
                            ld      a,(p?_XX15X2hi)            ; LDA XX15+5             \ Set A = x2_hi (use b as a substibute for a)
                            ld      b,a                     ; .
...
p?_LL147_6502:        ld      a,191                   ; LDX #Y*2-1             \ Set X = #Y * 2 - 1. The constant #Y is 96, the y-coordinate of the mid-point of the space view, so this sets Y2 to 191, the y-coordinate of the bottom pixel row of the space view
                            ld      (Xreg),a                ; .
;                    ld      a,127
;                    ld      c,a
.CheckX2Y2High:             ld      a,b                     ; ORA XX12+1             \ If one or both of x2_hi and y2_hi are non-zero, jump
                            ld      hl,p?_XX15Y2hi    ; .
                            or      (hl)                    ; .
                            jp      nz,p?_LL107_6502  ; BNE LL107              \ to LL107 to skip the following, leaving X at 191
.CheckY2Lo:                 ld      a,127 ;,c               ; get back the temporary x reg from c
                            ld      hl,p?_XX15Y2lo    ; CPX XX12               \ If y2_lo > the y-coordinate of the bottom of screen (a is being used as X at this point still)
                            cp      (hl)                    ; .
                            jp      c,p?_LL107_6502   ; BCC LL107              \ then (x2, y2) is off the bottom of the screen, so skip the following instruction, leaving X at 127
                            ZeroA                           ; LDX #0                 \ Set X = 0
                            ld      (Xreg),a            
p?_LL107_6502:        ld      a,(Xreg)                ; STX XX13               \ Set XX13 = X, so we have * XX13 = 0 if x2_hi = y2_hi = 0, y2_lo is on-screen* XX13 = 191 if x2_hi or y2_hi are non-zero or y2_lois off the bottom of the screen
                            ld      ( p?_XX13),a      ; now c is released as a temporary x reg
                            ld      a,(p?_XX15X1hi)   ; LDA XX15+1             \ If one or both of x1_hi and y1_hi are non-zero, jump
                            ld      hl,p?_XX15Y1hi    ; ORA XX15+3             \ to LL83
                            or      (hl)                    ; .
                            jp      nz,p?_LL83_6502            ; BNE LL83
; DEBUG SIMPLIFIED CODE, now we just compare y1 lo > 127
                            ...
                            jp      nc, p?_LL83_6502         ; BCC LL83               \ . (y1 > 127 jump, i.e. 127 <= y1 )
                            ld      a,( p?_XX13)                ; LDA XX13               \ If we get here, (x1, y1) is on-screen. If XX13 is non-zero, i.e. (x2, y2) is off-screen, jump
...
p?_LL146_6502:        ld      hl,(p?_XX15X1lo)           ;  Save X1 to XX1510
                            call    p?_ClampX
                            ld      (p?_XX15X1lo),a
                            ld      hl,(p?_XX15Y1lo)           ;  hl = y1
                            call    p?_ClampY
                            ld      (p?_XX1510+1),a            ;  XX1510... = [X1][Y1]
                            
                            ld      hl,(p?_XX15X2lo)           ;  de = x2
                            call    p?_ClampX
                            ld      (p?_XX1510+2),a            ;  XX1510... = [X1][Y1][X2]
                            
                            ld      hl,(p?_XX15Y2lo)           ;  bc = y2
                            call    p?_ClampY
                            ld      (p?_XX1510+3),a            ;  XX1510... = [X1][Y1][X2][Y2]
                            ClearCarryFlag                  ; CLC                    \ Clear the C flag as the clipped line fits on-screen
                            ret                             ; RTS                    \ Return from the subroutine
p?_LL109_6502:        SetCarryFlag                    ; SEC                    \ Set the C flag to indicate the clipped line does not fit on-screen
                            ret                             ; RTS                    \ Return from the subroutine
p?_LL108_6502:        ld      hl, p?_XX13                 ; LSR XX13               \ If we get here then (x2, y2) is off-screen and XX13 is
                            srl     (hl)                    ;                        \ 191, (128)  so shift XX13 right to halve it to 95 (64)
;-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;-- LL145 (Part 2 of 4)                               
p?_LL83_6502:         ld      a,( p?_XX13)               ; LDA XX13               \ If XX13 < 128 then only one of the points is on-screen
                            and     a                                ; BPL LL115              \ so jump down to LL115 to skip the checks of whether
                            jp      p,p?_LL115_6502            ;                        \ both points are in the strips to the right or bottom of the screen
;-- If we get here, both points are off-screen
                            ld      a,(p?_XX15X1hi)            ; LDA XX15+1             \ If both x1_hi and x2_hi have bit 7 set, jump to LL109
                            ld      hl,p?_XX15X2hi             ; AND XX15+5             \ to return from the subroutine with the C flag set, as
                            and     (hl)
                            jp      m, p?_LL109_6502           ; BMI LL109              \ the entire line is above the top of the screen
                            ld      a,(p?_XX15Y1hi)            ; LDA XX15+3             \ If both y1_hi and y2_hi have bit 7 set, jump to LL109
                            ld      hl,p?_XX15Y2hi             ; AND XX12+1             \ to return from the subroutine with the C flag set, as
                            and     (hl)                             ; BMI LL109              \ the entire line is to the left of the screen
                            jp      m,p?_LL109_6502            ; .
                            ld      a,(p?_XX15X1hi)            ; LDX XX15+1             \ Set A = X = x1_hi - 1
                            dec     a                                ; DEX
                            ld      (Xreg),a                         ; TXA
                            ...
                            ld      hl,p?_XX15Y2hi             ; ORA XX12+2             \ If neither (x1_hi - 1) or (x2_hi - 1) have bit 7 set,
                            or      (hl)                             ; .
                            jp      p, p?_LL109_6502           ; BPL LL109              \ jump to LL109 to return from the subroutine with the C flag set, as the line doesn't fit on-screen
...
p?_LL83_DEBUG:          ld      a,(p?_XX1532)              ; LDA XX15+2             \ If y1_lo < y-coordinate of screen bottom, clear the C
                            cp      128                     ; CMP #Y*2               \ flag, otherwise set it (NOTE FLIPPED IN z80)                   
                            ld      a,(p?_XX1532+1)            ; LDA XX15+3             \ Set XX12+2 = y1_hi - (1 - C), so:
                            sbc     0                       ; SBC #0                 \ .
                            ld      (p?_XX12p2),a              ; STA XX12+2             \  * Set XX12+2 = y1_hi - 1 if y1_lo is on-screen * Set XX12+2 = y1_hi  otherwise We do this subtraction because we are only interested
                            ld      a,(p?_XX1576)              ; LDA XX12               \ If y2_lo < y-coordinate of screen bottom, clear the C
                            cp      128                     ; CMP #Y*2               \ flag, otherwise set it
                            ld      a,(p?_XX1576+1)            ; LDA XX12+1             \ Set XX12+2 = y2_hi - (1 - C), so:
                            sbc     0                       ; SBC #0                   * Set XX12+1 = y2_hi - 1 if y2_lo is on-screen  * Set XX12+1 = y2_hi     otherwise
                            ld      hl,p?_XX12p2               ; ORA XX12+2             \ If neither XX12+1 or XX12+2 have bit 7 set, jump to
                            or      (hl)                    ; .
                            jp      p,p?_LL109_6502            ; BPL LL109              \ LL109 to return from the subroutine with the C flag set, as the line doesn't fit on-screen
;-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
;-- LL145 (Part 3 of 4) Summary: Clip line: Calculate the line's gradient
p?_LL115_6502:        ld      a,(Yreg)                ; TYA                    \ Store Y on the stack so we can preserve it through the call to this routine
                            push    af                      ; PHA            SP+1    \ call to this subroutine
                            ld      hl,(p?_XX15X2lo)             ; LDA XX15+4             \ Set XX12+2 = x2_lo - x1_lo
                            ld      de,(p?_XX15X1lo)             ; LDA XX15+5             \ Set XX12+3 = x2_hi - x1_hi
                            ClearCarryFlag                  ; SBC XX15+1
                            sbc     hl,de                   ; .
                            ld      (delta_x),hl            ; .
                            ld      hl,(p?_XX15Y2lo)             ; LDA XX12               \ Set XX12+4 = y2_lo - y1_lo
                            ld      de,(p?_XX15Y1lo)             ; 
...
; So we now have:  delta_x in XX12(3 2)  delta_y in XX12(5 4) where the delta is (x1, y1) - (x2, y2))
                            ...
                            ld      a,h
                            and     a
                            jp      p, p?_LL110_6502  ; BPL LL110              \ the following
                            NegHL                           ; LDA #0                 \ Otherwise flip the sign of delta_y to make it
                            ld      (delta_y),hl            ; positive, starting with the low bytes
p?_LL110_6502:        ld      hl,(delta_x)            ; LDA XX12+3             \ If delta_x_hi is positive, jump down to LL111 to skip
...
p?_LL111_6502:        ld      hl,(delta_x)
                            ld      de,(delta_y)
                            ld      a,h                     ; TAX                    \ If |delta_x_hi| is non-zero, skip the following
                            or      d                       ; BNE LL112
                            jp      z,p?_LL113_6502            ; LDX XX12+5             \ If |delta_y_hi| = 0, jump down to LL113 (as both |delta_x_hi| and |delta_y_hi| are 0)
p?_LL112_6502:        ShiftHLRight1                   ; LSR A                  \ Halve the value of delta_x in (A XX12+2)
                            ...
                            jp       p?_LL111_6502                  ; JMP LL111              \ Loop back to LL111
;-- By now, the high bytes of both |delta_x| and |delta_y| are zero
p?_LL113_6502:        ZeroA                           ; STX T                  \ We know that X = 0 as that's what we tested with a BEQ  above, so this sets T = 0
                            ld      (Tvar),a
                            ...
                            cp      (hl)
                            jp      c, p?_LL114_6502  ; BCC LL114              ; if delta y > delta x then its a steep slope so we do 256*dy/dx
;-- If we get here then our line is more horizontal than vertical, so it is a shallow slope
                            ...
                            call    p?_LL28_6502      ; JSR LL28               \ Call LL28 to calculate: R = 256 * A / Q = 256 * delta_y_lo / delta_x_lo
                            jp      p?_LL116_6502     ; JMP LL116              \ Jump to LL116, as we now have the line's gradient in R
;-- If we get here then our line is more vertical than horizontal, so it is a steep slope
p?_LL114_6502:        ld      a,(delta_y)             ; LDA XX12+4             \ Set Q = delta_y_lo
                            ld      (Qvar),a                ; STA Q
                            ld      a,(delta_x)             ; LDA XX12+2             \ Set A = delta_x_lo
                            call    p?_LL28_6502               ; JSR LL28               \ Call LL28 to calculate: R = 256 * A / Q = 256 * delta_x_lo / delta_y_lo
...
p?_LL116_6502:        ld      a,(Rvar)                ; LDA R                  \ Store the gradient in XX12+2
                            ld      (p?_XX12p2),a     ; STA XX12+2
                            ld      a,(Svar)                ; LDA S                  \ Store the type of slope in XX12+3, bit 7 clear means
                            ld      (p?_XX12p3),a              ; STA XX12+3             \ top left to bottom right, bit 7 set means top right to bottom left
                            ld      a,( p?_XX13)                ; LDA XX13               \ If XX13 = 0, skip the following instruction
                            cp      0                       ; BEQ LL138
                            jp      z,p?_LL138_6502            ; .
                            jp      p, p?_LLX117_6502          ; If XX13 is positive, it must be 95 (64) as 128 would be negative). This means (x1, y1) is on-screen but (x2, y2) isn't, so we jump        
;-- If we get here, XX13 = 0 or 191, so (x1, y1) is off-screen and needs clipping
p?_LL138_6502:          call    p?_LL118_6502              ; JSR LL118              \ Call LL118 to move (x1, y1) along the line onto the screen, i.e. clip the line at the (x1, y1) end
                            ld      a,( p?_XX13)                ; LDA XX13               \ If XX13 = 0, i.e. (x2, y2) is on-screen, jump down to
                            and     a
                            jp      p,p?_LL124_6502            ; BPL LL124              \ LL124 to return with a successfully clipped line
;-- If we get here, XX13 = 191 (128) (both coordinates areoff-screen)
p?_LL117_6502:          ld      a,(p?_XX1510+1)            ; LDA XX15+1             \ If either of x1_hi or y1_hi are non-zero, jump to
                            ld      hl,p?_XX1532+1             ; ORA XX15+3             \ LL137 to return from the subroutine with the C flag
                            or      (hl)
                            jp      nz, p?_LL137_6502          ; BNE LL137              \ set, as the line doesn't fit on-screen
                            ld      a,(p?_XX1532)              ; LDA XX15+2             \ If y1_lo > y-coordinate of the bottom of the screen
                            cp      128                     ; CMP #Y*2               \ jump to LL137 to return from the subroutine with the
                            jp      nc, p?_LL137_6502          ; BCS LL137              \ C flag set, as the line doesn't fit on-screen
...
p?_LLX117_6502:         ld      hl,( p?_XX1510)             ; LDX XX15               \ Swap x1_lo = x2_lo
                            ld      de,( p?_XX1554)
                            ld      ( p?_XX1510),de
                            ld      ( p?_XX1554),hl
                            ld      hl,( p?_XX1532)             ; LDX XX15+2             \ Swap y1_lo = y2_lo
                            ld      de,( p?_XX1576) 
                            ld      ( p?_XX1532),de
                            ld      ( p?_XX1576),hl
                            call    p?_LL118_6502              ; JSR LL118              \ Call LL118 to move (x1, y1) along the line onto the screen, i.e. clip the line at the (x1, y1) end
                            ld      hl,SWAP
...
p?_LL124_6502:          pop     af                      ; PLA            SP+0    \ Restore Y from the stack so it gets preserved through
                            ld      (Yreg),a                ; TAY                    \ the call to this subroutine
                            call    p?_LL146_6502              ; JMP LL146              \ Jump up to LL146 to move the low bytes of (x1, y1) and (x2, y2) into (X1, Y1) and (X2, Y2), and return from the subroutine with a successfully clipped line
                            ret                             ; then exit so we don't pop it twice
p?_LL137_6502:          pop     af                      ; PLA            SP+0    \ Restore Y from the stack so it gets preserved through
                            ld      (Yreg),a                ; TAY                    \ the call to this subroutine
                            SetCarryFlag                    ; SEC                    \ Set the C flag to indicate the clipped line does not fit on-screen
                            ret                             ; RTS                    \ Return from the subroutine


p?_l2_draw_6502_line:   ld      hl,x1                           ; copy from currnet position to 6502 variables
                            ld      de, p?_XX1510
                            ld      bc,4*2
                            ldir        
                            call    p?_LL145_6502                      ; perform 6502 version
