...
;ALIGN 2             ; This next table must start on an even address
WordPtr             DW  WordDummy,          WordFront,        WordRight,        WordLeft        ;0-3
...
WordPtrGalactic     DW  WordGalactic    ;24
                    DW  WordExtra,          WordClean,        WordOffender,     WordFugitive    ;28
WordPtrStock        DW  WordFood,           WordTextiles,     WordRadioactives, WordSlaves      ;32
...
WordPtrUoMFull      DW  WordTonnes,         WordKilograms,    WordGrams                         ;46
WordPtrShorRangeCht DW  WordShort,          WordRange                                           ;49
WordPtrChart        DW  WordChart                                                               ;51
WordPtrData         DW  WordData,           WordOn                                              ;52
                    ...
                    DW  WordMCR                                                                 ;66
WordPtrEconomy      DW  WordRich,           WordAvg,          WordMainly,       WordPoor        ;67
WordPtrIndustrial   DW  WordIndustrial                                                          ;71
WordPtrAgricultural DW  WordAgricultural                                                        ;72
WordPtrGovernment   DW  WordAnarchy,        WordFeudal,       WordMultiGovernment, WordDictatorship ;73
                    DW  WordCommunist,      WordConfederacy,  WordDemocracy,       WordCorporate ;77
                    DW  WordState
WordPtrLight        DW  WordLight
WordPtrYears        DW  WordYears      
WordPtr0            DW  Word0         ; 81
WordPtrMarketmenu   DW  WordMarket,         WordPrices,       WordProduct,         WordUoM      ;84
                    DW  WordPrice,          WordFor,          WordSale                          ;91
WordPtrUomAbbrev    DW  Wordt,              Wordkg,           Wordg                             ;94 
WordPtrQuantity     DW  WordQuantity,       WordInv,          WordStock
WordPtrEquip        DW  WordEquip   ;97
WordPtrShip         DW  WordShip,           WordMissile       ; 101
WordPtrShipNames:   DW  WordAdder,          WordAnaconda,     WordAsp_Mk_2,        WordBoa
...
WordIdx             EQU (WordPtr                      ) /2
WordIdxGalactic     EQU (WordPtrGalactic     - WordPtr) /2
WordIdxStock        EQU (WordPtrStock        - WordPtr) /2
WordIdxUoMFull      EQU (WordPtrUoMFull      - WordPtr) /2
WordIdxShorRangeCht EQU (WordPtrShorRangeCht - WordPtr) /2
WordIdxChart        EQU (WordPtrChart        - WordPtr) /2
WordIdxData         EQU (WordPtrData         - WordPtr) /2
WordIdxEconomy      EQU (WordPtrEconomy      - WordPtr) /2
WordIdxIndustrial   EQU (WordPtrIndustrial   - WordPtr) /2
WordIdxAgricultural EQU (WordPtrAgricultural - WordPtr) /2
WordIdxGovernment   EQU (WordPtrGovernment   - WordPtr) /2
WordIdxLight        EQU (WordPtrLight        - WordPtr) /2
WordIdxYears        EQU (WordPtrYears        - WordPtr) /2
WordIdx0            EQU (WordPtr0            - WordPtr) /2
WordIdxMarketmenu   EQU (WordPtrMarketmenu   - WordPtr) /2
WordIdxUomAbbrev    EQU (WordPtrUomAbbrev    - WordPtr) /2
WordIdxQuantity     EQU (WordPtrQuantity     - WordPtr) /2
WordIdxEquip        EQU (WordPtrEquip        - WordPtr) /2
WordIdxShip         EQU (WordPtrShip         - WordPtr) /2
WordIdxShipNames:   EQU (WordPtrShipNames    - WordPtr) /2
...
TextRichIndustrial      DB  WordIdxEconomy,       WordIdxIndustrial                 ,0 ;15
TextAvgIndustrial       DB  WordIdxEconomy+1,     WordIdxIndustrial                 ,0 ;16
TextMainIndustrial      DB  WordIdxEconomy+2,     WordIdxIndustrial                 ,0 ;17
TextPoorIndustrial      DB  WordIdxEconomy+3,     WordIdxIndustrial                 ,0 ;18
...
TextDummy               DB  0
; fast index to the token table above
...
TextGovOffset           EQU WordIdxGovernment
...
;takes a value in a and loads hl with the address for the corresponding string   
WordIndexToAddress:     ld      hl,WordPtr
WordLookup:             add     hl,a
                        add     hl,a
                        ld      a,(hl)
                        inc     hl
...
expandTokenToString:    ld      hl,TextTokens           ; get the address of the token set for the string
                        call    getTableText            ; hl = table, a = offset (in words), hl = result
                        ld      de,TextBuffer           ; de = target location to write text to
.ReadLoop:              ld      a,(hl)                  ; read index byte,
                        cp      0                       ; 0 means end of message
                        jr      z,.ReadDone             ; .
